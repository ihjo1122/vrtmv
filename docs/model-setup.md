...
## 1. VLM — Gemma 4 E2B-IT (.litertlm)

### 개요
- **모델**: Gemma 4 E2B-IT (INT4, 약 2.5GB)
- **엔진**: LiteRT-LM 0.11.0 (`litertlm-android`) — Multi-Token Prediction (MTP)으로 모바일 GPU 디코드 >2× 가속
- **호스팅**: 사용자 개인 private 미러 리포 (`HF_TOKEN` 필요) — Google 공식 리포는 gated이므로 `DownloadManager`로 직접 접근 불가
  - `https://huggingface.co/joinhyeok/gemma/resolve/main/gemma-4-E2B-it.litertlm`

### 자동 다운로드 (기본)
앱 최초 실행 시 **`IntroScreen`이 순차 다운로드 큐를 실행**하여 VLM 모델과 YOLO 자산을 자동으로 내려받는다. 진행률은 `[n/2] 항목명` 헤더와 함께 `GradientProgressBar`로 노출.
...
```bash
adb push gemma-4-E2B-it.litertlm /sdcard/Download/vrtmv/
```
...
- **Intro 자동 다운로드** (`getExternalFilesDir/vrtmv-assets/`): YOLO 모델
...
- 비전 입력: 최대 256px로 리사이즈 (Gemma 4 비전 인코더 입력 해상도, Gemma 3n 와 동일), JPEG 품질 90
...
- OBJECT_DETECTION / OBJECT_DETECTION_NO_PADDING 모드는 `CascadeDetectionPipeline`이 MediaPipe + YOLO 모두 사용 (사용자 선택 UI 없음). FULL_FRAME 모드는 검출 없이 `describeScene` 직행.
